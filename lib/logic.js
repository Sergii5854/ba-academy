...
/**
 * Sample transaction
 * @param {ba_academy.models.transactions.TransactionTrackTime} tx
 * @transaction
 */
function trackTime(tx) {
  const asset = getFactory().newResource(
    "ba_academy.models.assets",
    "AssetTimeEntry",
    uuid4()
  );
  ...
  return getAssetRegistry("ba_academy.models.assets.AssetTimeEntry")
    .then((registry) => {
      registry.add(asset);
    })
    .then(() => {
    ...
    .then(() => {
      return getAssetRegistry(
        "ba_academy.models.assets.AssetTimeEntry"
      );
    })
